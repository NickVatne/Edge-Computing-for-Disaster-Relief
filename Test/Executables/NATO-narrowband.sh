#!/bin/bash
echo 'NATO Narrowband Waveform Network Test 16kbit/s 20ms delay'
echo 'Add loss'
read -p 'Loss: ' loss 
echo 'Gilbert Elliot Model parameters <p>[<r>[<1-h>[<1-k>]]]'
read -p 'Format(1 5 10 10): ' p r h k
echo 'Adding Network Emulation'
sudo tc qdisc add dev wlan0 root netem rate 16kbit loss $loss% delay 20ms
...
echo 'Writing'
sudo tcpdump -i wlan0 -s 8883 port not 22 -w NATO_Narrowband_waveform_$loss%loss_$p%_$r%_$h%_$k%.pcap -c 5000
